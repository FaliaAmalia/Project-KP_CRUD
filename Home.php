...
class Home extends AUTH_Controller {
    ...
    public function index() {
        ...
        foreach ($delivery as $value) {
            ...
            $get_by_delivery = $this->M_deliverymenu->select_by_jmlh($value->id_delivery);

            $data_pengiriman[$index]['value'] = $get_by_delivery->jmlh;
            $data_pengiriman[$index]['color'] = $color;
            $data_pengiriman[$index]['highlight'] = $color;

            $data_pengiriman[$index]['host'] = $value->id_delivery;

            $index++;
        }


        // $deliverys = $this->M_deliverymenu->select_all();
        // $index = 0;
        // foreach ($deliverys as $value) {
        //     $color = '#' . $rand[rand(0, 15)] . $rand[rand(0, 15)] . $rand[rand(0, 15)] . $rand[rand(0, 15)] . $rand[rand(0, 15)] . $rand[rand(0, 15)];

        //     $guest_by_id = $this->M_deliverymenu->select_by_idDeliv($value->id_delivery);

        //     // $data_pengiriman[$index]['value'] = $guest_by_id->jmlh;
        //     $data_pengiriman[$index]['color'] = $color;
        //     $data_pengiriman[$index]['highlight'] = $color;
